...
function totalize () {
  var totalPrice = 0;
  var countTotal = 0;
  for (var i = 0, len = tbody.rows.length - 1; i < len; i++) {
...
function allSelect(e) {
  if (e.target.checked) {
    for (var i = 1, len = tbody.rows.length; i < len; i++) {
...
function changeSum(e) {
  ...
  switch (e.target.className) {
    ...
    case "cut":
      countList[rowIndex - 1].children[1].innerText--;
      if (rowIndex < tbody.rows.length - 2 
...
totalize ();
//给表格绑定点击事件，点击后刷新总价
table.addEventListener("click", changeSum);
